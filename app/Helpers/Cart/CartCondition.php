...
use App\Helpers\Cart\Validators\CartConditionValidator;
...
class CartCondition
{
    ...
    public function __construct(array $args)
    {
        ...
        if (YokartHelper::isMultiArray($args)) {
    ...
    {
        return $this->args['totalProducts'];
    }
    ...
    {
        return $this->args['attributes'] = $attributes;
    }
    ...
    protected function apply($totalOrSubTotalOrPrice, $conditionValue)
    {
        ...
        if ($this->valueIsPercentage($conditionValue)) {
            if ($this->valueIsToBeSubtracted($conditionValue)) {
                $value = YokartHelper::normalizePrice($this->cleanValue($conditionValue));
                ...
                $value = YokartHelper::normalizePrice($conditionValue);
        ...
        // next is we will check if it has a minus/plus sign so then we can just deduct it to total/subtotal/price
            ...
            if ($this->valueIsToBeSubtracted($conditionValue)) {
                $this->parsedRawValue = YokartHelper::normalizePrice($this->cleanValue($conditionValue));
                ...
                $this->parsedRawValue = YokartHelper::normalizePrice($conditionValue);
