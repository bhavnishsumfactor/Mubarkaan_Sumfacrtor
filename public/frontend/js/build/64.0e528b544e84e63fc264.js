...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  props: ["product", "rating", "sharethisNetworks", "pageUrl", "sizeExist", "allVarients", "selectedVarient", "otherVarients"],
  methods: {
    getVarient: function getVarient(varientCode) {
      this.$emit("selectedVarient", varientCode); // let formData = this.$serializeData({
      //     'product-id': this.product.prod_id,
      //     'varientCode': varientCode,
      // });
      // this.$axios
      //     .post(baseUrl + "/product/product-varient-price", formData)
      //     .then((response) => {
      //         $('.YK-price').html(response.data.data.price);
      //         if (response.data.data.sliderHtml) {
      //             destroyCarousel();
      //             $('.YK-image-slider').html(response.data.data.sliderHtml);
      //         }
      //         $('.YK-addTocartHtml').html(response.data.data.addToCartHtml);
      //         if (response.data.data.oldprice == 0) {
      //             $('.YK-oldprice').html('');
      //         } else {
      //             $('.YK-oldprice').html('<del>' + response.data.data.oldprice + '</del>');
      //         }
      //         $('.YK-oldprice del').html(response.data.data.oldprice);
      //         window.__sharethis__.initialize();
      //     });
    },
    decreaseQty: function decreaseQty(e) {
      if (this.$productStockVerify(this.product) == false) {
        return false;
      }
    },
    increaseQty: function increaseQty(e) {
      if (this.$productStockVerify(this.product) == false) {
        return false;
      }
    },
    addToCart: function addToCart() {
      if (this.$productStockVerify(this.product) != false && this.shipping == true) {
        this.$addToCart(this.product.prod_id, this.product.pov_code, false);
      } else {
        return false;
      }
    },
      ...
      _c("div", { staticClass: "product-detail__price" }, [
        ...
        _c("div", { staticClass: "price-wrappar" }, [
          _c(
            ...
            [_vm._v(_vm._s(_vm.product.display_finalprice))]
          ),
          _vm._v(" "),
          _vm.product.price != _vm.product.finalprice
            ? _c("div", { staticClass: "sale-price" }, [
                _c("del", [_vm._v(_vm._s(_vm.product.display_price))]),
                _vm._v(" "),
                      ...
                      _c("span", { staticClass: "spl-price" }, [
                        _vm._v(
                          _vm._s(_vm.$t("BDG_SAVE")) +
                            " " +
                            _vm._s(_vm.product.display_discount) +
                            "% "
                        )
                      ])
                    ])
                        ...
                        _c(
                          ...
                          _vm._l(_vm.selectedVarient.options, function(
                            ...
                            return _c("span", { key: "soindex_" + soindex }, [
                              ...
                              _vm._v(
                                "\n                                " +
                                  _vm._s(selectedoption.displayOpnValue) +
                                  "\n                                " +
                                  _vm._s(_vm.selectedVarient.displayPipeline) +
                                  "\n                            "
                              )
                            ])
                          }),
                                            ...
                                            return _c(
                                              ...
                                              [
                                                ...
                                                _vm._v(
                                                  "\n                                            " +
                                                    _vm._s(
                                                      selectedoption.displayOpnValue
                                                    ) +
                                                    "\n                                            " +
                                                    _vm._s(
                                                      varient.displayPipeline
                                                    ) +
                                                    "\n                                        "
                                                )
                                              ]
