//category collection layout 2 functions
//open setting popup
$('iframe').contents().find("body").on('click', '.yk-categoryCollectionLayout2 .yk-categories > *', function(e) {
    var $link = $(e.target);
    e.preventDefault();
    if (!$link.data('lockedAt') || +new Date() - $link.data('lockedAt') > 300) {
        setTimeout(function() {
            ...
            editSettings(cid, 'categoryCollectionLayout2');
        }, 100);
    }
    $link.data('lockedAt', +new Date());
});
//embed data into view
$(document).on('click', '.yk-addCategoryCollectionLayout2Widget', function(e) {
    let thisObj = $(this);
    thisObj.addClass('gb-is-loading').attr('disabled', 'disabled');
    let cid = thisObj.closest('.yk-categoryCollectionLayout2-settings').attr('data-comp');
    let selectedCategoriesCount = thisObj.closest('.yk-categoryCollectionLayout2-settings').find('.yk-selectedCategories li.yk-category').length;
    if (selectedCategoriesCount <= 6) {
        $.ajax({
            url: adminBaseUrl + '/widgets/load',
            type: 'POST',
            data: { 'layout': 'categoryCollectionLayout2', cid: cid },
            success: function(response) {
                let model = '';
                if (typeof editor.getSelected() != 'undefined') {
                    model = editor.getSelected();
                } else {
                    model = editor.DomComponents.getWrapper().find('section.yk-categoryCollectionLayout2')[0];
                }
                var responseEl = $(response);
                responseEl.find('.yk-categoryCollectionLayout2 .yk-container').attr('compid', cid);
                model.components(responseEl.html());
                resetComponentSettings(model, 'yk-categoryCollectionLayout2');
        ...
        thisObj.removeClass('gb-is-loading').removeAttr('disabled');
        toastr.error(atmost6CategoriesCanBeSelectedMessage);
    }
});
function embedCategoryCollection2(){
    let thisObj = $('.yk-addCategoryCollectionLayout2Widget');
    let cid = thisObj.closest('.yk-categoryCollectionLayout2-settings').attr('data-comp');
    $.ajax({
        url: adminBaseUrl + '/widgets/load',
        type: 'POST',
...
//autocomplete
$(document).on('click', '.yk-categoryCollectionLayout2-settings .yk-autocompleteCategories', function() {
    let thisObj = $(this);
...
$(document).on('change', '.yk-categoryCollectionLayout2-settings input.yk-autocompleteCategories', function() {
    let thisObj = $(this);
    let cid = thisObj.closest('.yk-categoryCollectionLayout2-settings').attr('data-comp');
    if (thisObj.val() != '') {
        let selectedCount = thisObj.closest('.yk-categoryCollectionLayout2-settings').find('.yk-selectedCategories li.yk-category').length;
        if (selectedCount >= 6) {
            thisObj.removeClass('gb-is-loading').removeAttr('disabled');
            toastr.error(atmost6CategoriesCanBeSelectedMessage);
            thisObj.val('');
        ...
        let alreadyExists = 0;
        $.each(thisObj.closest('.yk-categoryCollectionLayout2-settings').find('.yk-selectedCategories li[data-id]'), function() {
        ...
        let displayOrder = parseInt(thisObj.closest('.yk-categoryCollectionLayout2-settings').find('.yk-selectedCategories').attr('data-highest-order')) + 1;
        $.ajax({
            method: "POST",
            url: adminBaseUrl + '/collection/save',
            dataType: "json",
            data: { cid: cid, layout: 'categoryCollectionLayout2', records: selectedCategoryId, display_order: displayOrder },
            success: function(response) {
                if (response.status == false) {
                    toastr.error(response.message);
                } else {
                    thisObj.closest('.yk-categoryCollectionLayout2-settings').find('.yk-selectedCategories').append(`
        ...
        <div class="actions">
            <button type="button" class="btn btn-icon yk-removecategoryCollectionLayout2">
                <svg>   
                    ...
                    thisObj.closest('.yk-categoryCollectionLayout2-settings').find('.yk-selectedCategories').attr('data-highest-order', displayOrder);
                }
            }
        });
        thisObj.val('');
...
$(document).on('click', '.yk-removecategoryCollectionLayout2', function(e) {
    let thisObj = $(this);
    ...
    $.ajax({
        ...
        dataType: "json",
        data: { cid: cid, layout: 'categoryCollectionLayout2', record: categoryId },
        success: function(response) {
            ...
            embedCategoryCollection2();
        }
    });
});
/*remove background image*/
$(document).on('click', '.yk-categoryCollectionLayout2-settings .js-removeBackgroundImage', function(e) {
    let thisObj = $(this);
    let cid = thisObj.closest('.yk-categoryCollectionLayout2-settings').attr('data-comp');
    ...
    $.ajax({
        ...
        type: 'POST',
        data: { cid: cid, 'layout': 'categoryCollectionLayout2', record_id: recordId },
        success: function(response) {
            toastr.success(response.message);
...
//open cropper modal
$(document).on('click', '.yk-categoryCollectionLayout2-settings .js-openCropper', function(e) {
    window.tmppath = '';
    window.actualImage = '';
    window.actualImage = $(this).closest('.yk-slide').find('img.actualImage').attr('src');
    $(this).closest('.yk-categoryCollectionLayout2-settings').find('#modal_cropper input[type="file"]').val('');
    $(this).closest('.yk-categoryCollectionLayout2-settings').find('#modal_cropper input[name="tabId"]').val($(this).closest('.yk-category').attr('data-id'));
    window.aspectRatio = 1;
    if ($('.cropperSelectedImage').hasClass('cropper-hidden')) {
        window.$image.cropper('destroy');
    }
    if (window.actualImage == '') {
        $('.yk-categoryCollectionLayout2-settings .js-cropperSelectImage').trigger('click');
        return;
    }
    $('#settingsModal').modal('hide');
...
$(document).on('change', '.yk-categoryCollectionLayout2-settings input.js-cropperSelectImage', function(e) {
    if (e.target.files.length > 0) {
        selectImage(e.target.files[0]);
    }
});
//crop image
$(document).on('click', '.yk-categoryCollectionLayout2-settings .js-cropImage', async function(e) {
    let thisObj = $(this);
    thisObj.addClass('gb-is-loading').attr('disabled', 'disabled');
    let formData = new FormData();
    formData.append('cid', thisObj.closest('.yk-categoryCollectionLayout2-settings').attr('data-comp'));
    let tabId = thisObj.closest('#modal_cropper').find('input[name="tabId"]').val();
    formData.append('layout', 'categoryCollectionLayout2');
    ...
    cropped.toBlob((croppedBlob) => {
        ...
        $.ajax({
            ...
            success: function(response) {
                toastr.success(response.message);
                thisObj.closest('.yk-categoryCollectionLayout2-settings').find('.yk-category[data-id="' + tabId + '"] .actualImage').attr('src', response.data.originalUrl);
                thisObj.closest('.yk-categoryCollectionLayout2-settings').find('.yk-category[data-id="' + tabId + '"] .croppedImage').attr('src', response.data.url).closest('.YK-preview').show();
