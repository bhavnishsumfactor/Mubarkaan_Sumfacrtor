    ...
    <div class="subheader" id="subheader">
      <div class="container">
        <div class="subheader__main">
          <h3 class="subheader__title">{{ $t('LBL_SUB_ADMINS') }}</h3>
          <div class="subheader__breadcrumbs">
            ...
            <span class="subheader__breadcrumbs-separator"></span>
            <span class="subheader__breadcrumbs-link">{{$t('LBL_SUB_ADMINS')}}</span>
          </div>
        </div>
        <div class="subheader__toolbar">
          <router-link
            :to="{name: 'addSubAdmins'}"
            class="btn btn-white"
            v-bind:class="[(!$canWrite('ADMIN_USERS')) ? 'disabledbutton': '']"
          >
      ...
      <div class="row">
        <div class="col-xl-12">
          <div class="portlet portlet--height-fluid">
                ...
                <div class="section__content">
                  <table class="table table-data table-justified">
                    <thead>
                      <tr>
                        <th>{{'#' }}</th>
                        <th>{{ $t('LBL_ADMIN_NAME') }}</th>
                        <th>{{ $t('LBL_ADMIN_USERNAME')}}</th>
                        <th>{{ $t('LBL_ADMIN_EMAIL')}}</th>
                        <th>{{ $t('LBL_PUBLISH')}}</th>
                        <th>{{ $t('LBL_ADMIN_ROLE')}}</th>
                        <th></th>
                      </tr>
                    </thead>
                    <tbody
                    ...
                    <tbody v-else>
                      ...
                      >
                        ...
                        <td>
                          ...
                          >{{ record.admin_name }}</a>
                          <div v-else>{{ record.admin_name }}</div>
                        </td>
                        ...
                        <td>
                          <label class="switch switch--sm remember-me">
                            <input
                              type="checkbox"
                              v-model="record.admin_publish"
                              @change="onSwitchStatus($event, record.admin_id)"
                              :disabled="(!$canWrite('ADMIN_USERS'))"
                            />
                            <span
                              v-b-tooltip.hover
                              :title="record.admin_publish == 1 ? $t('TTL_UNPUBLISH') : $t('TTL_PUBLISH') "
                            ></span>
                          </label>
                        </td>
                        <td>{{ record.admin_role == null ? '': record.admin_role.role_name }}</td>
                        <td>
                          <div class="actions">
                            <a
                              class="btn btn-light btn-sm btn-icon"
                              :href="adminBaseUrl + '/sub-user/login/'+ record.admin_id"
                              v-b-tooltip.hover
                            ...
                            >
                              <svg>
                                <use
                                  :xlink:href="adminBaseUrl+'/images/retina/sprite.svg#impersonate-icon'"
                                  :href="adminBaseUrl+'/images/retina/sprite.svg#impersonate-icon'"
                                />
                              </svg>
                            </a>
                            <router-link
                              :to="{name: 'editSubAdmin',params: {id: record.admin_id}}"
                              v-b-tooltip.hover
                              :title="$t('TTL_EDIT')"
                              class="btn btn-light btn-sm btn-icon"
                              v-bind:class="[!$canWrite('ADMIN_USERS') ? 'disabled no-drop': '']"
                            ...
                            <a
                              ...
                              :title="$t('TTL_DELETE')"
                              href="javascript:;"
                              @click.capture="confirmDelete($event, record.admin_id)"
                              :disabled="record.admin_role == null"
                              v-bind:class="[!$canWrite('ADMIN_USERS') ? 'disabled no-drop': '']"
                            >
                              <svg>   
            ...
            <div class="portlet__body" v-if="showEmpty == 1 && pageLoaded==1">
              <div class="get-started">
                ...
                <div class="no-content d-flex text-center flex-column mb-7">
                  <p>
                    {{ $t('LBL_CLICK_THE')}}
                    <router-link :to="{name: 'addSubAdmins'}" class="btn btn-brand btn-small">
                      <i class="fas fa-plus"></i>
                      {{ $t('BTN_ADD')}}
                    </router-link>
                    {{ $t('LBL_BUTTON_TO_CREATE_SUB_ADMINS') }}
                  </p>
                </div>
            ...
            <DeleteModel
              :deletePopText="$t('LBL_SUBADMIN_DELETE_CONFIRMATION_TEXT')"
              :recordId="recordId"
...
export default {
  ...
  methods: {
    ...
    pageRecords: function(pageNo, pageLoad = false) {
      ...
      this.$http
        .post(adminBaseUrl + "/sub-admins/list?page=" + pageNo, formData)
        .then(response => {
          this.records = response.data.data.admins.data;
          this.pagination = response.data.data.admins;
          this.loading = false;
    ...
    deleteRecord: function(recordId) {
      this.$http
        .delete(adminBaseUrl + "/sub-admins/" + recordId)
        .then(response => {
    ...
    onSwitchStatus: function(event, id) {
      ...
      this.$http
        .post(adminBaseUrl + "/sub-admins/status/" + id, formData)
        .then(response => {
          if (response.data.status == false) {
            toastr.error(response.data.message, "", toastr.options);
            return;
