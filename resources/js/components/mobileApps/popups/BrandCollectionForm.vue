<template>
  <b-modal id="brandCollection" size="lg" centered title="BootstrapVue" no-close-on-backdrop>
    <template v-slot:modal-header>
      ...
      <div
        class="alert-text"
      >{{collectionType=='brand-collection2' ? '3 items need to be added' : '2 items need to be added'}}</div>
    </div>
      ...
      <div class="dropdown-menu dropdown-menu-fit dropdown-menu-anim dropdown-suggestions">
        <ul class="nav nav--block">
          <perfect-scrollbar :style="'max-height: 200px'">
            <li class="dropdown-item nav__item" v-for="(brand, index) in brands" :key="index">
      ...
      > 
        ...
        >
          <div class="row align-items-center">
            ...
            <div class="actions">
              <button type="button" class="btn btn-icon" @click="deleteBrandCollection(sindex)">
                <svg>
...
export default {
  ...
  computed: {
    isComplete() {
      if (this.collectionType == "brand-collection2") {
        return this.selectedCollections.length == 3 && this.pageTitle != "";
      } else {
  ...
  methods: {
    ...
    closePopup: function(close = false) {
      if (close) {
        ...
        this.$bvModal.hide("brandCollection");
        return;
      }
      $("#brandCollection").show();
    },
    ...
    uploadImages: function() {
      ...
      formData.append("type", "appBrandCollection");
      $("#brandCollection").show();
    ...
    deleteBrandCollection: function(index) {
    ...
    searchBrands: function() {
      this.$http
        .post(adminBaseUrl + "/products/search-brands", {
      ...
      this.$http
        ...
        .then(response => {
          ...
          Object.keys(editData).forEach(key => {
            ...
            this.selectedCollections.push({
              record_id: editData[key].brand_id,
              name: editData[key].brand_name,
