    ...
    <div>
        <div class="subheader" id="subheader">
            <div class="container">
                <div class="subheader__main">
                    <h3 class="subheader__title">{{ $t('LBL_NOTIFICATION_TEMPLATES') }}</h3>
                    <div class="subheader__breadcrumbs">
                        ...
                        <span class="subheader__breadcrumbs-separator"></span>
                        <span class="subheader__breadcrumbs-link">{{$t('LBL_NOTIFICATION_TEMPLATES')}}</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="container">
            <div class="row">
                            ...
                            <div class="section mb-0">
                                <div class="section__content">
                                    <table class="table table-data table-justified">
                                        ...
                                        <tbody>
                                            <tr v-for="(record, index) in records" :key="record.ntpl_id" :data-id="record.ntpl_id">
                                                <td scope="row">{{ pagination.from+index }}</td>
                                                <td>
                                                    <a href="javascript:;" @click.prevent="editRecord(record)" v-if="!$canWrite('NOTIFICATION_TEMPLATES')">{{ record.ntpl_name }}</a>
                                                    <div v-else>{{ record.ntpl_name }}</div>
                                                </td>
                                                <td>
                                                    <div class="actions">
                ...
                <div class="col-md-6">
                    <div class="portlet portlet--height-fluid">
                        <div class="portlet__head" v-if="recordData.ntpl_id">
                        ...
                        <div class="portlet__body" v-if="recordData.ntpl_id != ''" v-bind:class="[(!$canWrite('NOTIFICATION_TEMPLATES')) ? 'disabledbutton': '']">
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                    <label>{{ $t('LBL_NOTIFICATION_TEMPLATE_NAME')}} <span class="required">*</span></label>
                                        <input type="text" v-model="recordData.ntpl_name" name="ntpl_name" v-validate="'required'" data-vv-as="$t('LBL_NOTIFICATION_TEMPLATE_NAME') " data-vv-validate-on="none" class="form-control" />
                                        <span v-if="errors.first('ntpl_name')" class="error text-danger">@{{ errors.first('ntpl_name') }}</span>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control" rows="5" cols="15" v-model="recordData.ntpl_body" name="ntpl_body" @keyup="showLength" v-validate="'required'" data-vv-as="$t('LBL_NOTIFICATION_BODY') " data-vv-validate-on="none">
                                    </textarea>
                                        <span class="small">{{currentLength}}/160</span>
                                        <span v-if="errors.first('ntpl_body')" class="error text-danger">@{{ errors.first('ntpl_body') }}</span>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <ul class="click-to-copy" v-if="recordData.ntpl_replacements">
                                        <li v-for="(tags, index) in JSON.parse(recordData.ntpl_replacements)" :key="index"
                        ...
                        <div class="portlet__body" v-if="recordData.ntpl_id == ''">
                        ...
                        <div class="portlet__foot" v-if="recordData.ntpl_id != ''">
                            <div class="row">
                                <div class="col">
...
<script>
    ...
    export default {
        ...
        computed: {
            isComplete () {
                return this.recordData.ntpl_name && this.recordData.ntpl_body;
            }
        },
        methods: {
            ...
            pageRecords: function(pageNo, pageLoad = false) {
                ...
                this.$http.post(adminBaseUrl + '/notification-templates/list?page=' + pageNo, formData).then((response) => {
            ...
            showLength: function() {
                this.currentLength = this.recordData.ntpl_body.replace(/\{([^}]+)\}/g, "").length;
            },
            ...
            validateRecord: function() {
                this.$validator.validateAll().then(res => {
                    if (res) {
                        ...
                        this.clicked = 1;
                        if (input.ntpl_id != '') {
                            this.updateRecord(input.ntpl_id, input);
                        }
                    } else {
                        this.clicked = 0;
                    }
            ...
            editRecord: function(input) {
                ...
                this.recordData.ntpl_id = input.ntpl_id;
                this.recordData.ntpl_name = input.ntpl_name;
                this.recordData.ntpl_body = input.ntpl_body;
                this.recordData.ntpl_default_body = input.ntpl_default_body;
                this.recordData.ntpl_replacements = input.ntpl_replacements;
                this.editText = input.ntpl_name;
                this.currentLength = this.recordData.ntpl_body.replace(/\{([^}]+)\}/g, "").length;
                if (input.updated_at != null && "admin_username" in input.updated_at) {
                    this.updatedByUser = input.updated_at.admin_username;
                }
                this.updatedAt = input.ntpl_updated_at ? input.ntpl_updated_at : '';
            },
            updateRecord: function(id, input) {
                let formData = this.$serializeData({
                    'ntpl_id': input.ntpl_id,
                    'ntpl_name': input.ntpl_name,
                    'ntpl_body': input.ntpl_body
                });
                formData.append('_method', 'put');
                this.$http.post(adminBaseUrl + '/notification-templates/' + id, formData)
                    .then((response) => {
                        this.clicked = 0;
            ...
            resetBody: function() {
                this.recordData.ntpl_body = this.recordData.ntpl_default_body;
            },
            onSwitchStatus: function(event, id) {
                let formData = this.$serializeData({
                    'status': event.target.checked
                });
                this.$http.post(adminBaseUrl + '/notification-templates/status/' + id, formData).then((response) => {
                    toastr.success(response.data.message, '', toastr.options);
                });
            },
