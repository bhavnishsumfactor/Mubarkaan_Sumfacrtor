        ...
        <div class="subheader" id="subheader">
            <div class="container">
                <div class="subheader__main">
                    <h3 class="subheader__title">{{ (!$canWrite('PICKUP_ADDRESS') ? '' : $t('LBL_EDIT')) }}  {{ $t('LBL_PICKUP_ADDRESSES') }}</h3> </div>
            </div>
        </div>
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-lg-8">
                    <!--begin::Portlet-->
                    <form class="form form--label-right" v-on:submit.prevent="validateRecord" >
                        <div class="portlet" id="page_portlet">
                                ...
                                <div class="section">
                                    <div class="section__body">
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-lg-8">
                                                <vue-tel-input v-if="defaultCountryCode !=''"  v-model="adminsData.saddr_phone" :defaultCountry="defaultCountryCode" :enabledCountryCode="true" @country-changed="changeCountry" @input="onPhoneNumberChange" inputClasses="form-control" placeholder="Enter a phone number" :validCharactersOnly="true" :maxLen="15"></vue-tel-input>
                                            ...
                                            <div class="col-auto">
                                                <div class="form-group">
                                                    <div class="radio-inline">
                                                        <label class="radio">
                                                            <input type="radio" name="radiopickup" :value="2" v-model="pickupStatus" @click="changeShopTiming(2)"/> {{ $t('LBL_OPEN_ALL_DAYS') }} <span></span> 
                                                        </label>
                                                        <label class="radio">
                                                            <input type="radio" checked="checked" :value="1" v-model="pickupStatus" name="radiopickup" @click="changeShopTiming(1)"/> {{ $t('LBL_SELECT_SHOP_OPEN_TIMINGS') }} <span></span> 
                                                        </label>
                                                        <label class="radio">
                                                            <input type="radio" name="radiopickup" :value="3" v-model="pickupStatus" @click="changeShopTiming(3)"/> {{ $t('LBL_OPEN_EXCEPT_WEEKENDS') }} <span></span> </label>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group row" v-if="pickupStatus == 2 || pickupStatus == 3">
                                            ...
                                            <div class="row">
                                                <div class="col-lg-6 col-xl-6">
                                                    <select class="form-control" v-model="adminsData.start_time" name="start_time" v-validate="'required'" :data-vv-as="$t('LBL_START_TIME')" data-vv-validate-on="none" @change="fetchEndTimes">
                                                        <option value="" disabled>{{ $t('LBL_SELECT_TIMING')}}</option>
                                                        <option v-for="(time, timeIndex) in startTimes" :key="timeIndex" :value="time">{{time}}</option>
                                                    </select> <span v-if="errors.first('start_time')" class="error text-danger">{{ errors.first('start_time') }}</span> </div>
                                                <div class="col-lg-6 col-xl-6">
                                                    <select class="form-control" v-model="adminsData.end_time" name="end_time" v-validate="'required'" :data-vv-as="$t('LBL_END_TIME')" data-vv-validate-on="none">
                                                        <option value="" disabled>{{ $t('LBL_SELECT_TIMING')}}</option>
                                                        <option v-for="(time, timeIndex) in endTimes" :key="timeIndex" :value="time">{{ time }}</option>
                            ...
                            <div class="portlet__foot">
                                <div class="row">
                                    ...
                                    <div class="col-auto">
                                        <button type="submit" class="btn btn-wide btn-brand gb-btn gb-btn-primary" :disabled="clicked==1 || isComplete || (!$canWrite('PICKUP_ADDRESS'))" v-bind:class="clicked==1 && 'gb-is-loading'">{{ $t('BTN_PICKUP_UPDATE')}}</button>
                                    </div>
                                </div>
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        ...
        updateRecord: function(input) {
            ...
            this.$http.post(adminBaseUrl + '/store-address/' + input.saddr_id, formData)
        ...
        addDays : function(day){
            this.days[day].timeslots.push({start_time:'',end_time:''});
        },
        pageLoadData: async function() {
            let id = this.$route.params.id;
            await this.$http.get(adminBaseUrl + '/store-address/' + id + '/edit')
                .then((response) => { //
                    this.countries = response.data.data.countries;
                    this.states = response.data.data.states;
                    this.adminsData = response.data.data.record;
                    [this.startTime,this.endTime] = [response.data.data.record.start_time, response.data.data.record.end_time];
                    this.pickupStatus = this.adminsData.saddr_shop_open_status;                    
                    this.days = response.data.data.timings;
                    ...
                    setTimeout(function() {
                        ...
                        if (this.pickupStatus == 2 || this.pickupStatus == 3) {
                            this.adminsData.start_time = response.data.data.start_time;
                            this.adminsData.end_time = response.data.data.end_time;
                        }
                    }, 1000);
                 });
        },
        ...
        changeShopTiming: function(shopStatus) {
            if (this.adminsData.saddr_shop_open_status != shopStatus) {
                this.adminsData.start_time = '';
                this.adminsData.end_time = '';
            } else {
                this.adminsData.start_time = this.startTime;
                this.adminsData.end_time = this.endTime;
            }
        },
