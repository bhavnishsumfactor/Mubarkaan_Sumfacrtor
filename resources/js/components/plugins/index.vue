							...
							<div class="portlet__body pb-0 bg-gray flex-grow-0">
								<div class="form-group">
									<!--<div class="input-icon input-icon--left">
										<input type="text" class="form-control" :placeholder="$t('PLH_SEARCH')" id="generalSearch" v-model="searchData.search" @keyup="searchRecords"> <span class="input-icon__icon input-icon__icon--left">
                                    <span><i class="la la-search"></i></span> </span> <span class="input-icon__icon input-icon__icon--right" v-if="searchData.search!=''" @click="searchData.search='';pageRecords(1);">
                                    <span><i class="fas fa-times"></i></span> </span>
									</div>-->
								</div>
							</div>
							<hr class="m-0">
												...
												<tr v-for="(record, index) in records" :key="index">
													...
													<td>
														<label class="switch switch--sm">
															<input type="checkbox" v-model="record.pkg_publish" @change="onSwitchStatus($event, record.pkg_id, record.pkg_card_type)"> <span v-b-tooltip.hover :title="record.pkg_publish == 1 ? $t('TTL_UNPUBLISH') : $t('TTL_PUBLISH') "></span> </label>
													</td>
													<td>
														<a class="btn btn-light btn-sm btn-icon " href="javascript:;" v-b-tooltip.hover :title="$t('TTL_EDIT')" @click.prevent="editRecord(record)"> <i class="fas fa-cog"></i> </a>
													</td>
												</tr>
											</tbody>
										</table>
						...
						<div class="portlet portlet--height-fluid">
							<div class="portlet__head" v-if="selectedPage">
								<div class="portlet__head-label">
									<h3 class="portlet__head-title" v-if="selectedPage == 'editform'">{{ $t('LBL_SETTING') }} - {{adminsData.pkg_name}}</h3> </div>
							</div>
							<div class="portlet__body" v-if="selectedPage">
								<div class="row">
									<div class="col-md-12">
										...
										<div class="form-group">
											<input type="text" v-model="adminsData.pkg_name" name="pkg_name" v-validate="'required'" :data-vv-as="$t('LBL_PAYMENTMETHOD_NAME')" data-vv-validate-on="none" class="form-control" /> <span v-if="errors.first('pkg_name')" class="error text-danger">{{ errors.first('pkg_name') }}</span> </div>
									</div>
								</div>
								<div class="row" v-for="(configuration, index) in configurations" :key="'config'+index">
									<div class="col-md-12">
										...
										<div class="form-group">
											<input type="text" v-model="configuration.value" :name="configuration.key" v-validate="'required'" :data-vv-as="$t(configuration.name.replace('_',' '))" data-vv-validate-on="none" class="form-control" /> <span v-if="errors.first(configuration.key)" class="error text-danger">{{ errors.first(configuration.key) }}</span> </div>
									</div>
								</div>
							</div>
							<div class="portlet__body" v-if="selectedPage == ''">
