<template>
    <div>
        <div class="datatable datatable--default datatable--brand datatable--loaded" id="sessionByDevice" v-if="records.length > 0">
            <table class="datatable__table">
                <thead class="datatable__head datatable__head-fixed">
                    <tr class="datatable__row">
                        <th class="datatable__cell datatable__cell--sort" @click="sortBy('deviceCategory')" v-bind:class="[
                            sortingBy == 'deviceCategory' ? 'datatable__cell--sorted' : '']">
                            <span>{{ $t('LBL_DEVICE_TYPE') }}
                                <i class="fas" v-bind:class="[sortingType == 'asc' ? 'fa-arrow-up' : 'fa-arrow-down']" v-if="sortingBy == 'deviceCategory'"></i>
                            </span>
                        </th>
                ...
                <tbody class="datatable__body">
                    ...
                    <tr class="datatable__row" v-for = "(item, key) in records" :key="key">
                        <td class="datatable__cell"><span>{{ item.device_type ? item.device_type : '' }}</span></td>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            printSessionByDevice() {
                this.$htmlToPaper('sessionByDevice');
            },
