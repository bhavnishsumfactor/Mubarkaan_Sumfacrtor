...
<div class="signup-full">
    ...
    <div class="signup-full-body">
        <div class="flow-cms">
            <h1>{{__('LBL_ACCOUNT_VERIFICATION_HEADING')}}</h1>
            
                @if($step == "email-verify")
                    <p>{{__('LBL_ACCOUNT_VERIFICATION_EMAIL_INSTRUCTIONS')}} {{$user->user_email}}</p>
                @elseif($step == "phone-verify")
                    <p>{{__('LBL_ACCOUNT_VERIFICATION_PHONE_INSTRUCTIONS')}} {{$user->user_phone}}</p>      
                @endif
...
verifyOtp = function() {
    ...
    $.post("{{route('verifyCode')}}", {'slug': slug, 'otp' : otp, 'type' : loginType, "_token": "{{ csrf_token() }}" }, function (response) {
        ...
        if (response.status == true) {
            ...
            $("#errorInfo").html(`<i class="fas fa-info-circle" data-toggle="tooltip" data-placement="top" title="${response.message}"></i>`);
