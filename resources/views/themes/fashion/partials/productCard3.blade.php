...
$altValue = $productImage->afile_attribute_alt;
$titleValue = $productImage->afile_attribute_title;
}
if(empty($favorite)){
$favorite = 0;
}
...
@endphp
<div class="product product-2 @if($stock != true) no-stock-wrap @endif YK-productListing" data-ratio="{{$aspectRatio}}">
    @if($stock != true)
    <div class="no-stock">{{__("LBL_OUT_OF_STOCK")}}</div>
    @endif
    <div class="product-body @if(getConfigValueByName('BACKGROUND_COLOR_ENABLED') == 0 && $bgColour != 0){{'outline'}}@endif">
        <div class="product-image @if($bgColour != 0){{'js-fillcolor'}}@endif">
