@if($updateListing == 0)
<div class="yk-blogLayout2-settings" data-comp="{{$cid}}">
        ...
        <div class="modal-dialog modal-dialog-centered" data-yk=""  role="yk-document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{__('LBL_BLOG_ARTICLES')}}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="alert alert-secondary" data-yk=""  role="yk-alert">
                        <div class="alert-icon"><i class="flaticon-questions-circular-button"></i></div>
                        <div class="alert-text">{{__("LBL_BLOG_ARTICLES_INSTRUCTION1")}}<br>
                            {{__("LBL_BLOG_ARTICLES_LAYOUT2_INSTRUCTION2")}}
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="input-icon input-icon--left">
                            <input type="text" class="form-control yk-autocompleteblogLayout2" placeholder="{{__('PLH_SEARCH_BLOG_POSTS')}}..." name="blog_name" autocomplete="off">
                    ...
                    @endphp
                    <ul class="list-group mt-4 yk-selectedBlogs yk-sortable" data-highest-order="{{$highestOrder}}">     
                        @if(!empty($data))
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-brand gb-btn gb-btn-primary yk-addBlogLayout2Widget">{{__('BTN_EMBED')}}</button>
                </div>
            </div>
        </div>
    </div>
