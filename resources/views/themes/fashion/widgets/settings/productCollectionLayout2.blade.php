@if($updateListing == 0)
<div class="yk-productCollectionLayout2-settings" data-comp="{{$cid}}">
        ...
        <div class="modal-dialog modal-dialog-centered" data-yk=""  role="yk-document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{__('LBL_PRODUCT_LAYOUT2')}}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="alert alert-secondary" data-yk=""  role="yk-alert">
                        <div class="alert-icon"><i class="flaticon-questions-circular-button"></i></div>
                        <div class="alert-text">{{__('LBL_PRODUCT_LAYOUT2_INSTRUCTIONS')}}</div>
                    </div>
                    <div class="form-group">
                        <div class="input-icon input-icon--left">
                            <input type="text" class="form-control yk-autocompleteProductsLayout2" placeholder="{{__('LBL_SEARCH_PRODUCTS')}}..." name="prod_name" autocomplete="off">
                    ...
                    @endphp
                    <ul class="list-group mt-4 yk-selectedProducts yk-sortable" data-highest-order="{{$highestOrder}}">
                        @if(!empty($data))
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-brand gb-btn gb-btn-primary yk-addProductCollectionLayout2Widget">{{__('BTN_EMBED')}}</button>
                </div>
            </div>
        </div>
    </div>
