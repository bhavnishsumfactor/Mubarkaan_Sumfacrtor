<div class="yk-promotionalBannerCollection3-settings" data-comp="{{$cid}}">
    ...
    <div class="modal-dialog modal-dialog-centered" data-yk=""  role="yk-document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">{{__('LBL_PROMOTIONALBANNERCOLLECTION3')}}</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
          <div class="form-group row">
              <label class="col-md-4 col-form-label" for="validationCustom01">{{__("LBL_BUTTON_LINK_URL")}}</label>
              <div class="col-md-8"> <input type="text" class="form-control" name="link" value="{{$data['link'] ?? ''}}" required>
              </div>
          </div>
          <div class="form-group row">
              <label class="col-md-4 col-form-label">{{__("LBL_BUTTON_TARGET")}}</label>
              <div class="col-md-8"> <select class="custom-select my-1 mr-sm-2" name="target">
                      <option value='' readonly disabled selected>{{__("LBL_CHOOSE")}}...</option>
                      <option value="_self" @if(!empty($data['target']) && $data['target']=="_self" ) selected @endif>{{__("LBL_SAME_TAB")}}</option>
                      <option value="_blank" @if(!empty($data['target']) && $data['target']=="_blank" ) selected @endif>{{__("LBL_NEW_TAB")}}</option>
                  </select>
              </div>
          </div>
          <ul class="list-group">
              <li class="list-group-item preview-wrapper yk-slide">
                <div class="row">
                  <div class="col">
                      ...
                      <p class="img-disclaimer editor"><strong>{{ __('LBL_BACKGROUND_IMAGE') }}: </strong> {{ __('LBL_IMAGE_RESTRICTIONS') }} 2000x500 {{__("LBL_IN")}} 4:1 {{ __('LBL_ASPECT_RATIO') }}</p>
                  </div>
      ...
      <div class="modal-footer">
          <button type="button" class="btn btn-brand gb-btn gb-btn-primary yk-addPromotionalBannerCollection3Widget">{{__('BTN_EMBED')}}</button>
      </div>
      </div>
    </div>
  </div>
