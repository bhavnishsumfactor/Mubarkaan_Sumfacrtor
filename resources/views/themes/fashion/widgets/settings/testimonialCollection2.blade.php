@if($updateListing == 0)
<div class="yk-testimonialCollection2-settings" data-comp="{{$cid}}">
<div class="modal fade" id="settingsModal" tabindex="-1" data-yk=""  role="yk-dialog" aria-labelledby="exampleModalLabel" style="display: none;" aria-hidden="true">
  ...
  <div class="modal-header">
    <h5 class="modal-title" id="exampleModalLabel">{{__('LBL_TESTIMONIALCOLLECTION2')}}</h5>
    <button type="button" class="close" data-dismiss="modal" aria-label="Close"></button>
  </div>
  <div class="modal-body">
    <div class="alert alert-secondary" data-yk=""  role="yk-alert">
      <div class="alert-icon"><i class="flaticon-questions-circular-button"></i></div>
      <div class="alert-text">{{__("LBL_TESTIMONIALCOLLECTION2_INSTRUCTIONS")}}</div>
    </div>
    <div class="form-group">
      <div class="input-icon input-icon--left">
        <input type="text" class="form-control yk-autocompleteTestimonialCollection2" placeholder="{{__('PLH_SEARCH_TESTIMONIALS')}}..." name="testimonial_name" autocomplete="off">        
    ...
    @endphp
    <div class="yk-perfectScrollbar">
    <ul class="list-group mt-4 yk-selectedTestimonials yk-sortable" data-highest-order="{{$highestOrder}}">     
      @if(!empty($data))
  ...
  <div class="modal-footer">
    <button type="button" class="btn btn-brand gb-btn gb-btn-primary yk-addTestimonialCollection2Widget">{{__('BTN_EMBED')}}</button>
  </div>
  </div>
  </div>
  </div>
