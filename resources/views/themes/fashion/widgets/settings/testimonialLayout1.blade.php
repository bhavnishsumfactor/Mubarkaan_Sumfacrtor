@if($updateListing == 0)
<div class="yk-testimonialLayout1-settings" data-comp="{{$cid}}">
<div class="modal fade" id="settingsModal" tabindex="-1" data-yk=""  role="yk-dialog" aria-labelledby="exampleModalLabel" style="display: none;" aria-hidden="true">
  ...
  <div class="modal-header">
    <h5 class="modal-title" id="exampleModalLabel">{{__('LBL_TESTIMONIAL')}}</h5>
    <button type="button" class="close" data-dismiss="modal" aria-label="Close"></button>
  </div>
  <div class="modal-body">
    <div class="alert alert-secondary" data-yk=""  role="yk-alert">
      <div class="alert-icon"><i class="flaticon-questions-circular-button"></i></div>
      <div class="alert-text">{{__("LBL_TESTIMONIAL_INSTRUCTIONS")}}</div>
    </div>
    <div class="form-group">
      <div class="input-icon input-icon--left">
        <input type="text" class="form-control yk-autocompletetestimonialLayout1" placeholder="{{__('PLH_SEARCH_TESTIMONIALS')}}..." name="testimonial_name" autocomplete="off">        
  ...
  <div class="modal-footer">
    <button type="button" class="btn btn-brand gb-btn gb-btn-primary yk-addTestimonialLayout1Widget">{{__('BTN_EMBED')}}</button>
  </div>
  </div>
  </div>
  </div>
